...
class MarketAnalysisService {
    calculateStatistics(precos) {
        ...
        if (!precos || precos.length === 0)
            return { media: 0, mediana: 0, desvioPadrao: 0, precosValidos: [] };
    ...
    classifyPrices(precos, mediana) {
        return precos.map(p => {
            const valor = Number(p.valor_unitario);
            const dataColeta = (0, moment_1.default)(p.data_coleta);
